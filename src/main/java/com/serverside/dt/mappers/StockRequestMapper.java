...
public interface StockRequestMapper {
    ...
    @Mapping(source = "stock.companyName", target = "company")
    ...
    @Mapping(source = "accountStock.averagePrice", target = "avgCost")
    @Mapping(source = "accountStock.purchasedDate", target = "purchaseDate", dateFormat = "yyyy-MM-dd")
    ...
    // @Mapping(source = "latestDetails.dividendYield", target = "dividendYield") // waiting for entity
    ...
    StockRequestDTO toDto(
            Account account,
            AccountStock accountStock,
            Stock stock,
            Currency currency,
            DividendFrequency frequency,
            StockDividendDetails latestDetails,
            List<StockDividendDetails> allDetails
    );

    default List<String> mapPayoutDates(List<StockDividendDetails> details) {
        return details.stream()
                .map(StockDividendDetails::getPayoutDate)
